...
**Reason:** the whole reconcile path was additive. `install.sh` and `sync.sh` only ever *add* or *relink*, and `brew bundle install` only ever *installs*, so retiring a managed config stranded fossils on every already-provisioned machine: ADR-034 dropped tmux and left `~/.config/tmux/tmux.conf` dangling with its directory behind, and the pull that prompted this work turned up two more orphans the same way (`~/.config/mc/ini`, `~/.config/ghostty/themes`). A broken symlink pointing into `$DOTFILES` is the one thing safe to auto-remove: a live `links.map` row would have relinked it on this very run, so a survivor is provably the fossil of a deleted row, and it is provably dead. A broken link pointing anywhere else is someone else's and is left untouched. The tap change is forced by newer Homebrew refusing formulae from untrusted third-party taps: a fresh `brew bundle` aborts on `felixkratz/formulae` → `borders`, and `brew bundle` does not auto-trust, so the taps must be trusted first.
**The vault-sync bug, in full:** the prior code ran `mkdir -p "$_src"` on the primary vault's `.obsidian/themes`, then moved each secondary vault's *real* `themes`/`plugins` to `.bak` and symlinked them at the primary. On a Google Drive vault the source shows as an online-only placeholder that `stat`s as a directory and even lists entries, yet a symlink to it does not resolve. So `[ -d "$_src" ]` and `ls -A "$_src"` both pass while the resulting link dangles — the target's real content is replaced by a link to nothing. Observed live twice during this session, each time eating the personal vault's Catppuccin theme and style-settings plugin. No cheap precheck survives cloud lazy-materialization, so the only reliable guard is to attempt the link and verify it resolves, rolling the backup straight back when it does not.
**Rejected:** `brew bundle cleanup` to handle package removal symmetrically. It uninstalls everything not in the Brewfile, which would take out the deliberately-unmanaged tools ADR-035 keeps installed on purpose (`gcloud-cli`, the hand-installed Keymapp). Package retirement stays a targeted by-hand step; only *symlinks we can prove are ours* are pruned automatically. Also rejected: a static precheck (`[ -d ]` + non-empty) for the vault source, which is exactly what the placeholder defeats.
**Note:** the prune is scoped to a fixed root list rather than all of `$HOME` on purpose — it keeps the walk cheap and stays well clear of the Google Drive vault trees, whose symlinks the vault-sync manages under its own resolve-or-rollback rule. The `test.sh` check is empty in CI (no managed symlinks under `$HOME` there) and on any synced machine, so it never false-fails; it exists to catch this exact drift on a real machine before a commit.
**Extends:** ADR-034 (links.map guards, tmux retirement), ADR-035 (unmanaged-on-purpose tools), ADR-033 (AeroSpace/borders + felixkratz tap)

---

## ADR-041: `aerospace.toml` gets test coverage; `⌥T` as a one-way escape from an accidental float
**Date:** 2026-08-16
**Decision:** `test.sh` gains three AeroSpace checks, all reading the tracked repo file rather than `~/.aerospace.toml`: the TOML parses; every `mode <name>` command resolves to a declared mode; and the `borders` invocation inside `after-startup-command` carries two `0xAARRGGBB` colours and a numeric width. They are skipped with the existing `⏭️` convention when `python3` is absent, matching the `ghostty`/`yq` guards. Separately `alt-t = 'layout tiling'` is added to the main mode, and the cheatsheet gains a `⌥T` row.
**Reason:** nothing validated this config until ADR-039 removed a binding mode, at which point the only available validator was AeroSpace itself. That is the wrong place to find a mistake: `auto-reload-config = true` means a broken file reaches the running window manager the instant it is saved, and `reload-config --dry-run` reads `~/.aerospace.toml`, so it can only check a change *after* it has already been symlinked live. The three checks are chosen for failures that are silent rather than loud. A dangling `mode` reference parses cleanly and surfaces only as a keypress that does nothing, which is precisely what ADR-039 risked by deleting a mode. Malformed `borders` arguments are not a parse error at all: they produce a focus highlight that fails to appear, and only at the next AeroSpace launch, long after the edit that caused it. `⌥T` addresses the fault that opened this whole sequence: `alt-shift-f` sits one Shift from `alt-f`, a slipped modifier floats a window silently, and a floating window ignores focus and move commands, which reads as AeroSpace having broken rather than as one window having changed state. `layout tiling` is idempotent (verified: repeat invocations exit 0 with "Already in the requested tiling mode"), unlike the `layout floating tiling` toggle, so it is safe to press when the cause is not yet understood.
**Also fixed:** `show-shortcuts.sh` hard-coded `DOTFILES="${HOME}/dotfiles"`, so it rendered the live checkout's sheets no matter where it ran from. Raycast always invokes the live copy, so this was invisible in normal use, but it meant `test.sh`'s two cheatsheet checks silently validated `~/dotfiles` rather than the tree under test — a worktree or a fresh clone could pass tests describing a file it does not contain. It now resolves the repo from `${BASH_SOURCE[0]}`, three levels up from `config/raycast/scripts/`, falling back to `~/dotfiles` if that yields no `config/` directory. Caught while confirming the new `⌥T` row rendered: it did not, because the renderer was reading the other checkout.
**Rejected:** shelling out to `aerospace reload-config --dry-run` from `test.sh`, which would tie the suite to a macOS machine with AeroSpace installed and running, break the GitHub Actions run, and still only validate the live symlink rather than the tracked file. Also rejected: rebinding `alt-shift-f` away from `alt-f` to prevent the slip. It would break existing muscle memory to solve a problem that a recovery key solves without taking anything away.
**Note:** corrects a factual error committed in ADR-039's changelog entry, which blamed the shadowing of `log` on a zshrc function. `log` is a zsh **builtin** (`whence -v log`); nothing in this repo shadows it, and `/usr/bin/log` is needed regardless.
**Extends:** ADR-039, ADR-040
...
**Amended same day (T124):** sage was rejected in use, twice, as still not readable at a glance, and the final values are `active_color=0xff00e676` (vivid green), `inactive_color=0x00000000` (fully transparent) and `width=8.0`. Three things were wrong with the sage decision. First, it optimised the wrong quantity: scoring the candidates as contrast × HLS saturation puts sage last of everything measured (7.20:1 × 20% = 1.4, against 11.1 for Mocha yellow and 9.8 for the vivid green chosen), because a desaturated colour reads as another shade of grey no matter how well it separates on luminance. Contrast was the right rebuttal to the original complaint and the wrong metric to then select on. Second, the width cut to 4.0 landed in the same commit and pulled directly against the goal, trading away highlight legibility to serve the gaps complaint. Third, and the actual fix, the strongest lever was never pulled: an alpha of `00` on the inactive colour means unfocused windows draw no border whatsoever, so exactly one border exists on screen and focus needs no comparison at all. That also *reduces* total border clutter versus any opaque inactive colour, which is why `width` could go back up to 8.0 without reopening the separation complaint. Gaps stay at 6.
...
**Reason:** a binding mode captures the whole keyboard, and `on-mode-changed = []` meant entering it produced no feedback of any kind. Inside resize mode only `h/j/k/l/b/↩/esc` did anything and every other key was silently swallowed, so the failure mode was indistinguishable from a frozen keyboard or a dead Voyager connection, with no discoverable way out short of knowing `esc`. The mode bought only explicit width-vs-height control over `resize smart`, which already picks the axis from the parent container's orientation, so the trade was a rarely-needed axis override against a keyboard that appears to hang.
**Rejected:** driving JankyBorders from `on-mode-changed` to colour the border while in resize mode, which was the first fix considered. It fails on shared state: `borders` is a single process updated by re-invocation (`man borders`), the restore command would have to hard-code an `active_color`, and T117's note that the border colour is "still being auditioned" is confirmed live — the running instance (started 12:54 on 15 Aug, ~4 h after AeroSpace, so not from `after-startup-command`) carries Macchiato `0xffe1e3e4`/`0xff494d64` at `width=6.0` against the toml's Mocha `0xffa6adc8`/`0xff313244` at `8.0`. Any restore would silently end that audition. Feedback via `osascript` notifications was rejected as a banner per mode switch. Keeping the mode and documenting `esc` harder was rejected because the cheatsheet already documented it and the trap still fired.
**Consequence:** no way to resize strictly across the split axis; `resize smart` follows the container, and `⌥/` flips the container first if the other axis is wanted. Reintroduce explicit `resize width`/`resize height` on a modeless binding if that turns out to bite.
**Note:** the border-colour drift above is left alone deliberately, it is the open audition from ADR-033, not a regression. Unrelated but found in the same pass: `brew bundle check` fails on `keymapp`, which is declared at `Brewfile:47` but installed by hand at `/Applications/Keymapp.app` and unknown to Homebrew, so it needs adopting rather than the Brewfile changing.
**Extends:** ADR-033 (AeroSpace config), ADR-038 (cheatsheet is the rendered source of truth)
...
**Amended same day (T121):** cheatsheets use macOS modifier symbols (`⌥⇧F`) with a legend line, not spelled-out modifiers, and the renderer gained `dwidth()` because macOS awk measures `length()` in bytes: each glyph is 3 bytes, so `%-10s` padding on `⌥⇧F` fell 6 spaces short and collapsed the column. Column width is now computed per table rather than per file, and `**bold**` becomes ANSI rather than leaking asterisks.
