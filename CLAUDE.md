...
**The agreed order governs the merge, not just the deploy.** When two sessions agree who merges what, "don't wait for me" is not permission to change `main` while a sibling is mid-run. If you want something kept out of someone else's deploy, hold the **merge**, not the ping — where the deploy checks out `main` at dispatch, merging *is* adding it to their deploy. If you land something anyway, say so immediately: a scope already reported to the user must be re-reported, never quietly widened (2026-08-21, PR #432).
...
**Linear is project tooling, not gated cloud MCP.** Creating issues, commenting, moving state, and setting relations are the documented workflow (`/triage`, `/start-task`, `/done`, `/autopilot` all write to Linear) and need no per-action approval — follow `conventions.md` › Work management instead. Recording a ruling the user just gave is a write, not a decision. The gated list above stays exhaustive: Gmail, Drive, Notion, Calendar. Ambiguity here had two sessions applying different standards on 2026-08-21.
...
**Evidence must be loud.** **Anything you run after a gate command (tests, lint, build, a CI watch) replaces its exit code** — a pipe into `tail`/`grep`, a chained `; tail -3`, even a bare `; echo "exit=$?"`. Redirect output to a file and end the command with the gate's own status (`RC=$?; …; exit $RC`), or run nothing after it. This bites hardest on background tasks, where the harness reports the *chain's* result and that notification is all you see: on 2026-08-20 a failed deploy was announced as "exit code 0" because a trailing `tail` had eaten the real 1, and two independent sessions hit the same trap that day, neither of them piping. Never rely on implicitly shared runtime state between concurrent sessions (compose project names, published ports, tagged images) — check for a live stack before booting one. **A CI run's conclusion is not proof its gates ran**: a job blocked before it starts (billing/spending limit, concurrency, cancellation) reports with **zero steps and no logs**, and the reason lives only in the check-run annotation, not the run log. Before merging on green, assert the legs you depend on have a non-zero step count.

**A negative result is only as good as its bounds.** Before reporting or acting on "nothing found", state what the query was bounded by — and when a peer reports a negative, ask the same before believing it. Two shapes lie confidently: a time-windowed log query (a `timestamp>=` filter that excludes the very window in question returns empty and reads as proof of absence), and a line-oriented grep for a multi-word phrase (long user-facing strings are wrapped across source lines, so no alternation can ever match — search a single distinctive token and read the context). Both happened on 2026-08-21; one produced a false accusation, the other a "clean" sweep reported to the user while two live defects sat on `main`.

**Never dispatch a deploy off an in-flight CI run.** Gate it on the **push run for `main` itself** having concluded green — not the PR's own checks, not the merge. Two PRs each green on their own base are never gated as a pair. And where the pipeline checks out `main` at dispatch time, whatever landed between your plan and your apply ships too, so re-read `main` immediately before dispatching and name the sha you actually shipped.
